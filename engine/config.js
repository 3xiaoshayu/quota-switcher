...
// The end-to-end smoke test points every upstream API at one local stub with
// CODEX_MANAGER_API_ORIGIN. Only the origin is swapped so the stub can route
// on the real paths; unset (every normal run) leaves the constants literal.
const API_ORIGIN_OVERRIDE = String(process.env.CODEX_MANAGER_API_ORIGIN || "").trim().replace(/\/+$/, "");

function upstreamUrl(url) {
  if (!API_ORIGIN_OVERRIDE) return url;
  return url.replace(/^https?:\/\/[^/]+/i, API_ORIGIN_OVERRIDE);
}

const CLIENT_ID = "app_EMoamEEZ73f0CkXaXp7hrann";
const AUTH_URL = "https://auth.openai.com/oauth/authorize";
const TOKEN_URL = upstreamUrl("https://auth.openai.com/oauth/token");
const USAGE_URL = upstreamUrl("https://chatgpt.com/backend-api/wham/usage");
...
const CURSOR_POLL_URL = upstreamUrl("https://api2.cursor.sh/auth/poll");
const CURSOR_TOKEN_URL = upstreamUrl("https://api2.cursor.sh/oauth/token");
const CURSOR_USAGE_URL = upstreamUrl("https://cursor.com/api/usage-summary");
const CURSOR_META_URL = upstreamUrl("https://api2.cursor.sh/aiserver.v1.AuthService/GetUserMeta");
...
const ANTIGRAVITY_TOKEN_URL = upstreamUrl("https://oauth2.googleapis.com/token");
const ANTIGRAVITY_USERINFO_URL = upstreamUrl("https://www.googleapis.com/oauth2/v2/userinfo");
const ANTIGRAVITY_CLOUDCODE_URL = upstreamUrl("https://cloudcode-pa.googleapis.com");
const ANTIGRAVITY_CLOUDCODE_DAILY_URL = upstreamUrl("https://daily-cloudcode-pa.googleapis.com");
...
module.exports = {
  ...
  CODEX_AUMID, resolveCodexHomeFromEnv, upstreamUrl,
};
